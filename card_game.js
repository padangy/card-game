...
function shuffle(deck) {
  ...
  while(currentIndex != 0) {
    ...
    temporaryValue = shuffledDeck[currentIndex]; // storing card at currentIndex to temporaryValue
    shuffledDeck[currentIndex] = shuffledDeck[randomIndex]; // reassigning card at currentIndex to randomIndex in shuffledDeck
    shuffledDeck[randomIndex] = temporaryValue; // reassigning card at randomIndex in shuffledDeck to temporaryValue
    currentIndex--;
  }
  return shuffledDeck;
}
...
function guess(card1, card2) {
  ...
  if(input === 'h'){
    ...
    console.log('You get no points for this round. Please guess either h or l.')
    return false
  }
}
...
function playGame(){
  ...
  while(score < 5 && score < deck.length) {
    ...
    if(guess(currentCard, nextCard) === true) {
      score++;
      console.log(playerName + ', you\'re correct! Your score is ' + score)
    } else{
      console.log('Sorry, you\'re wrong! You get no points. Your score is ' + score)
    }
    currentCard = nextCard;
  }

  deck.length === 0 ? console.log('You have no cards left. You lose.') : console.log('Congratulations, ' + playerName + ' you win the Card Game!')
}
